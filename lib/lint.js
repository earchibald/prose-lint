...
async function lint(text, { rules, jev = false, key, fetch: fetch_, cacheFile, min }) {
  ...
  for (const p of per) {
    ...
    for (const [id, q] of Object.entries(rules.questions)) {
      ...
      if (score >= (min !== undefined ? min : q.min)) findings.push({ line: p.s.line, sentence: p.s.text, rule: id, kind: 'judge', score: Math.round(score * 100) / 100, match: best, note: q.fix || '' });
    }
  }
