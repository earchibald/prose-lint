...
function proseLines(text) {
  ...
  text.split('\n').forEach((raw, i) => {
    ...
    raw = raw.replace(/!?\[([^\]]*)\]\([^)]*\)/g, '$1').replace(/\s*<?https?:\/\/[^\s)>]+>?/g, '');
...
function sentences(text) {
  ...
  for (const p of paragraphs(text)) {
    ...
    // A sentence starts with a capital, a digit, or code. It may also start with a lowercase file name, such as lib/message.js.
    let at = 0;
    for (const piece of masked.split(/(?<=[.?!]["')*_]*)\s+(?=["'(*_`]*(?:[A-Z0-9`]|[a-z][\w-]*[\/.][\w\/.-]*\w\s))/)) {
