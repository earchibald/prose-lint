...
function config(env = process.env) {
  const home = env.HOME || os.homedir(); // A value that still reads ${...} is a placeholder that nothing filled, so it counts as not set.
  const o = k => { const v = env['CLAUDE_PLUGIN_OPTION_' + k]; return v === undefined || /^\$\{.*\}$/.test(v) ? undefined : v; };
  let key = '', keyFrom = 'none';
  if (!env.PROSE_LINT_NO_JEV) {
    key = o('API_KEY') || env.TYPESAFE_API_KEY || ''; keyFrom = o('API_KEY') ? 'option' : key ? 'env' : 'none';
    // The key file is from the time before the plugin. It still works, so a terminal command can reach the judge.
    if (!key) try { const m = fs.readFileSync(path.join(home, '.config', 'prose-lint', 'env'), 'utf8').match(/^TYPESAFE_API_KEY=(.*)$/m); if (m) { key = m[1].trim().replace(/^["']|["']$/g, ''); keyFrom = 'file'; } } catch {}
  }
  return {
    key, keyFrom, jev: !env.PROSE_LINT_NO_JEV,
